...
export const ProductList = () => {
    ...
    const handleBrandFilters=(e)=>{
        ...
        // 1. For clicking the text links under New Arrivals (expects categoryId string)
        const handleCategoryFilterText = (categoryId) => {
            setFilters({ ...filters, category: [categoryId] });
            console.log("Filtering products by category ID:", categoryId);
        };

        // 2. For toggling the checkboxes inside the Accordion (expects event object)
        const handleCategoryFiltersCheckbox = (e) => {
            ...
            if (isChecked) {
                filterSet.add(targetValue);
            } else {
                filterSet.delete(targetValue);
            }
            ...
            <Stack mb={'5rem'}  sx={{scrollBehavior:"smooth",overflowY:"scroll"}}>
                    ...
                    <Stack mt={2}>
                        <Accordion>
                            ...
                            <AccordionDetails sx={{p:0}}>
                                <FormGroup onChange={handleCategoryFiltersCheckbox}>
                                    {
                                        categories?.map((category)=>(
                                            <motion.div style={{width:"fit-content"}} whileHover={{x:5}} whileTap={{scale:0.9}}>
